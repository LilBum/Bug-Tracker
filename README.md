...
Create a frontend environment file only if the API is not on the Vite proxy:
...
| Variable | Default | Description |
| --- | --- | --- |
| `VITE_API_BASE_URL` | empty | Optional API base URL. Leave empty for local development with the Vite proxy. |
...
| `npm run dev` | Alias for the Vite development server. |
...
- `vite is not recognized`: run `npm install` inside `frontend`.
...
- Production frontend cannot reach the API: set `VITE_API_BASE_URL` before
